...
function applyLanguage(languageCode) {
  ...
  function lookUp(key) {
    ...
    if (value === undefined) {
      console.warn('No "' + key + '" key exists in the ' + languageCode + ' translations.');
    }
...
function stopAutoplayingMedia() {
  ...
  document.querySelectorAll('iframe[src*="autoplay=1"]').forEach(function (frame) {
...
function setUpTextSizeControls() {
  ...
  function showCurrentSize() {
    buttons.forEach(function (button) {
      button.setAttribute('aria-pressed', String(button.dataset.textSize === root.dataset.textSize));
    });
  }
...
function setUpCommunityDropdown() {
  ...
  menu.addEventListener('focusout', function (event) {
    const movingWithinMenu = menu.contains(event.relatedTarget) || trigger.contains(event.relatedTarget);
    if (!movingWithinMenu) {
      closeMenu(false);
    }
  });
...
function markCurrentPage() {
  ...
  document.querySelectorAll('.nav-list a, .donate-button').forEach(function (link) {
